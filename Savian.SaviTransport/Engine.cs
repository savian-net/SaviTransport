...
namespace Savian.SaviTransport
{
    ...
    public class Engine
    {
        ...
        {
            if (string.IsNullOrEmpty(options.XptFile)) throw new Exception("XptFile is not set in options.");
            Common.Initialize(options);
            var fi = new FileInfo(Common.Options.XptFile);
            var lib = new SasXportLib();
            lib.Process(fi);
            if (!string.IsNullOrEmpty(options.OutputFile)) CreateOutputFile(lib);
        ...
        private static void CreateDelimitedFile(SasXportLib lib)
        {
            ...
            foreach (var sv in data.Variables) sw.Write(sv.Name + Common.Options.Delimiter);
            sw.WriteLine();
            ...
            foreach (var obs in data.Observations)
            {
                foreach (var cell in obs.Cells)
                    if (cell.Value != null)
                        sw.Write(cell.Value + Common.Options.Delimiter);
                    else
